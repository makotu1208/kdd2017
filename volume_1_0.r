...
if (predict == 1){
  for (i in 18:24){
    ...
    train_1 <- data_1_0[paste('2016-09-19 00:00::2016-10-',i,' 07:40:',sep = '')]
    test_1 <- data_1_0[paste('2016-10-',i,' 08:00::2016-10-',i,' 09:40:',sep = '')]
    ...
    model <- bsts(paste(colnames(train_1[1])," ~ holiday_special + rain_1_3 + rain_3 + before_special + typhoon_flag + before_long_vacation_flag + workday * dow",sep = "")
  ...
  for (i in 11:17){
    ...
    # validation making
    # 9m3wと10m2wの加重平均をテストデータにする
    
    test_base_1 <- data_1_0[paste('2016-10-',i,' 08:00::2016-10-',i,' 09:40:',sep = '')]
    test_base_1_all <- data_1_0[paste('2016-10-',i,' 08:00::2016-10-',i,' 14:40:',sep = '')]
    test_base_2 <- data_1_0[paste('2016-09-',i+9,' 08:00::2016-09-',i+9,' 09:40:',sep = '')]
    test_base_2_all <- data_1_0[paste('2016-09-',i+9,' 08:00::2016-09-',i+9,' 14:40:',sep = '')]
    #test_2[,1] <- NA
    ...
    test <- cbind(test_base_1[,1],test_base_2[,1])
    test <- test_base_1[,1] * 0.6 + test_base_2[,1] * 0.4
    test_all <- cbind(test_base_1_all[,1],test_base_2_all[,1])
    test_all <- test_base_1_all[,1] * 0.6 + test_base_2_all[,1] * 0.4
    ...
    model <- bsts(paste(colnames(train_1[1])," ~ holiday_special + rain_1_3 + rain_3 + before_special + typhoon_flag + workday * dow",sep = "")
                  ,state.specification = ss
                  ,niter=niter
                  ,contrasts=list(
                    holiday_special=contrasts(train_1$holiday_special)
    ...
    test_base_1[,1] <- test_base_1[,1]
    test_base_1_all[,1] <- test_base_1_all[,1]
    
    # inverse transform
    # test_2[,1] <- bcInverse(pred_1$mean,lam_1_0$roundlam)
    # test_base_1[,1] <- bcInverse(test_base_1[,1],lam_1_0$roundlam)
  ...
  time_1 = cbind(result_base_1[,1],result_1[,1])
  time_1 = na.omit(time_1)
  MAPE_am = mean(abs((time_1[,1]-time_1[,2])/time_1[,1]))
  print(MAPE_am)
  
  time_test = cbind(test,result_1[,1])
  time_test = na.omit(time_test)
  MAPE_am_test = mean(abs((time_test[,1]-time_test[,2])/time_test[,1]))
  print(MAPE_am_test)
  ...
  time_all_test = cbind(test_all,result_1[,1])
  time_all_test = na.omit(time_all_test)
  MAPE_am_test_all = mean(abs((time_all_test[,1]-time_all_test[,2])/time_all_test[,1]))
  print(MAPE_am_test_all)
...
if (predict == 1){
for (i in 18:24){
  train_2 <- data_1_0[paste('2016-09-19 00:00::2016-10-',i,' 16:40:',sep = '')]
  test_2 <- data_1_0[paste('2016-10-',i,' 17:00::2016-10-',i,' 18:40:',sep = '')]
  ...
  ss <- AddAr(ss, train_2[,1], 
              #lag = setting$bestlag[setting$day == i & setting$ampm==1]
              lag = 1
              )
  
  model <- bsts(paste(colnames(train_2[1])," ~ holiday_special + workday * rain_1_3 + workday * rain_3 + before_special + typhoon_flag + before_long_vacation_flag + workday * dow",sep = "")
                ,state.specification = ss
                ,niter=niter
                ,contrasts=list(
  ...
  for (i in 11:17){
    ...
    test_base_2 <- data_1_0[paste('2016-10-',i,' 17:00::2016-10-',i,' 18:40:',sep = '')]
    test_base_2_all <- data_1_0[paste('2016-10-',i,' 17:00::2016-10-',i,' 23:40:',sep = '')]
    ...
    model <- bsts(paste(colnames(train_2[1])," ~ holiday_special + before_long_vacation_flag + workday * rain_1_3 + workday * rain_3 + before_special + workday * dow",sep = "")
                  ,state.specification = ss
                  ,niter=niter
                  ,contrasts=list(
                    holiday_special=contrasts(train_2$holiday_special)
...
if (predict == 1){
  ...
  # 結合してデータを残す
  result_1_0 <- rbind(result_1_df,result_2_df)
  write.csv(result_1_0,"result_1_0.csv")
  save(result_1_0,file="result_1_0.RData")
} else {
  # 結合してMAPEを計算
  result_1_0 <- rbind(result_1,result_2)
  base_1_0 <- rbind(result_base_1,result_base_2)
  MAPE_v_1_0 = mean(abs((base_1_0[,1]-result_1_0[,1])/base_1_0[,1]))
  MAPE_v_1_0
  save(result_1_0,file="result_1_0_kensyo.RData")
  save(base_1_0,file="base_1_0_kensyo.RData")
  save(MAPE_v_1_0,file="MAPE_v_1_0.RData")
}
