...
if (predict == 1){
  for (i in 18:24){
    train_1 <- data_2_0[paste('2016-09-19 00:00::2016-10-',i,' 07:40:',sep = '')]
    test_1 <- data_2_0[paste('2016-10-',i,' 08:00::2016-10-',i,' 09:40:',sep = '')]
    ...
    model <- bsts(paste(colnames(train_1[1])," ~ holiday_special + typhoon_flag + before_special + workday",sep = "")
    ...
    for (i in 11:17){
      ...
      test_base_1 <- data_2_0[paste('2016-10-',i,' 08:00::2016-10-',i,' 09:40:',sep = '')]
      test_base_1_all <- data_2_0[paste('2016-10-',i,' 08:00::2016-10-',i,' 14:40:',sep = '')]
      #test_2[,1] <- NA
...
if (predict == 1){
for (i in 18:24){
  train_2 <- data_2_0[paste('2016-09-19 00:00::2016-10-',i,' 16:40:',sep = '')]
  test_2 <- data_2_0[paste('2016-10-',i,' 17:00::2016-10-',i,' 18:40:',sep = '')]
  ...
  model <- bsts(paste(colnames(train_2[1])," ~ holiday_special + rain_1_3 + rain_3 + before_special + workday * dow",sep = "")
                ,state.specification = ss
                ,niter=niter
                ,contrasts=list(
  ...
  for (i in 11:17){
    ...
    test_base_2 <- data_2_0[paste('2016-10-',i,' 17:00::2016-10-',i,' 18:40:',sep = '')]
    test_base_2_all <- data_2_0[paste('2016-10-',i,' 17:00::2016-10-',i,' 23:40:',sep = '')]
    ...
    model <- bsts(paste(colnames(train_2[1])," ~ holiday_special + rain_1_3 + rain_3 + workday * dow + before_special",sep = "")
                  ,state.specification = ss
                  ,niter=niter
                  ,contrasts=list(
                    holiday_special=contrasts(train_2$holiday_special)
...
if (predict == 1){
  ...
  result_2_df["tollgate_id"] = '2'
  result_2_df["direction"] = '0'
  
  # 結合してデータを残す
  result_2_0 <- rbind(result_1_df,result_2_df)
  write.csv(result_2_0,"result_2_0.csv")
  save(result_2_0,file="result_2_0.RData")
} else {
  # 結合してMAPEを計算
  result_2_0 <- rbind(result_1,result_2)
  base_2_0 <- rbind(result_base_1,result_base_2)
  MAPE_v_2_0 = mean(abs((base_2_0[,1]-result_2_0[,1])/base_2_0[,1]))
  MAPE_v_2_0
  save(result_2_0,file="result_2_0_kensyo.RData")
  save(base_2_0,file="base_2_0_kensyo.RData")
  save(MAPE_v_2_0,file="MAPE_v_2_0.RData")
}
